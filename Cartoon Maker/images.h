...
struct ColorImage {
	...
	sven::rgb * load(const char * fname, int *width, int * height);

	static int MainOfColor2grey(CString filename);
//	int MainOfColor2grey(CString filename)
};
...
struct GrayImage {
	...
	GrayImage( ColorImage &s) : data(new float[s.N]), w(s.w), h(s.h), N(s.N) {
		for(i=0;i<N;i++) 
			data[i]=(s.data)[i].l;			// s.data[i].l as in L*a*b*
	}
