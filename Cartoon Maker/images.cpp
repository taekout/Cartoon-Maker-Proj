...
float * ColorImage::calc_d() {
	float * d = new float [N];
	int i,j;
	for( i=0; i<N;i++) d[i] = 0;		// just initialization..
	...
	if(quantize) {
		...
		for( i=0; i<N;i++) for(j=i+1;j<N;j++) {
			float delta = calc_delta(i,j);		// guess the 'calc_delta' function is what they define the target difference 
			d[i]+=delta;
			d[j]-=delta;
		}
...
void GrayImage::complete_solve(const float * d) {
	for(i=1;i<N;i++) {								// Applying it to all pixels of the destination picture,
													// dest_pixel[i] = (the DELTA_source_pixel[i, i-1] + N * (dest_pixel[i-1]))) / N
...
void GrayImage::post_solve(const ColorImage &s) {
	float error=0;
	for(i=0;i<N;i++)						// compute the average error value of how difference the grey data has/have become
		error+=data[i]-(s.data)[i].l;		// compared to the original image
	error/=N;								// now it's an average erraneous value per pixel.
	for( i=0;i<N;i++) data[i]=data[i]-error;	//	Now I got it.. because of the adjustment(adding the contrast)
												//  the entire image has been a little darker than supposed.
												//  so now it looks as bright as it was in the colored image before.
												//  the above is not indicated in the paper[Color2Grey].
}
...
int ColorImage::MainOfColor2grey(CString filename)
{
  static int PARSE_ARGS_AMY = 1;
	
	//load in the arguments.
	
	const char * fname= new char[filename.GetLength()];
	::WideCharToMultiByte(CP_ACP, 0, filename, -1
			, (LPSTR)fname, filename.GetLength() + 1, NULL,NULL);
/*
	if (PARSE_ARGS_AMY)
	...
	ColorImage source;
	int width , height;
	source.load(fname, &width, & height);
...
//	clock_t start = clock();
	...
	float *d;
	if(r) {								// the value applied to the decision of localization = 0 or NOT??
		d = source.r_calc_d(r);
		dest.r_solve(d,r);
	}
	else {
		if(quantize) {					// if FULL image  ==>  check the value 'quantize'...
...
float ColorImage::calc_delta(int i, int j) const {
	...
	float dL=a.l-b.l;										//  delta L = the difference between an 'a' pixel and a 'b' pixel
	float dC= crunch(sqrt(sq(a.a-b.a)+sq(a.b-b.b)));		// delta C = ((Delta_A ^2) + (Delta_B^2))^1/2
															// By adjusting the delta C variable with the 'crunch' function, 

	if(fabsf(dL)>dC) return dL;								// Return whatever is bigger between the delta L and the delta C
															// Before comparing, get the absolute value of the delta L..
