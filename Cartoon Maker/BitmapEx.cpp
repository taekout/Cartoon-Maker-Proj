...
void CBitmapEx::Offset(long offsetX, long offsetY)
{
	...
	if (IsValid())
	{
		...
		long _offsetY = max(0, min(m_bih.biHeight/2-1, offsetY));
		long _offsetX = max(0, min(m_bih.biWidth/2-1, offsetX));
...
void CBitmapEx::GlowingEdges(long threshold, long scale)
{
	// Check for valid bitmap
	if (IsValid())
	{
	...
	if (IsValid() && (m_lpFire != NULL))
	{
		...
		for (i=0; i<m_bih.biHeight; i++)
		{
			...
			for (j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				lpDstData[dwTotalOffset>>2] = _RGB(m_lpFirePalette[m_lpFire[dwOffset]].rgbRed, m_lpFirePalette[m_lpFire[dwOffset]].rgbGreen, m_lpFirePalette[m_lpFire[dwOffset]].rgbBlue);
				dwOffset++;
